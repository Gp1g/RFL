...
def main():
    X, y, min_ = generate_synthetic(alpha=0, beta=0, iid=0)  # synthetiv (0,0)
    ...
    for i in trange(NUM_USER, ncols=120):
        ...
        train_data = {'x': np.array(X[i][server_len:train_len+server_len]), 'y': np.array(y[i][server_len:train_len+server_len])}
