...
def generate_emnist(dir_path, num_clients, num_classes, niid, balance, partition):
    ...
    trainset = torchvision.datasets.EMNIST(
        root=dir_path + "rawdata", split='byclass', train=True, download=True, transform=transform)
    testset = torchvision.datasets.EMNIST(
        root=dir_path + "rawdata", split='byclass', train=False, download=True, transform=transform)
...
def generate_server_test(dataset_image, dataset_label, num_classes):
    ...
    x = np.vstack([inner_data for inner_data in x]).reshape(num_classes * 100, 1, 28, 28)
